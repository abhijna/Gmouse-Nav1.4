...
pdf(file="~/Dropbox/Current_Biology/Gmouse-Nav1.4/Figures/Figure_6/Figure_6B.pdf")
...
current <- subset(beta, Species == "Gmouse" | Species == "Di" |  Species == "DiDiiiNC" )
...
peaks$Species <- factor(peaks$Species,levels = c("Gmouse", "Di", "DiDiiiNC"))
...
ggplot(peaks, aes(x = Species, y = Peak_Diff, fill = Species)) +
  ...
  scale_x_discrete(name = "", labels= c("Gmouse", "mDI", "Triple Mutant")) +
  coord_cartesian(ylim = c(0,0.5)) +
  scale_color_manual(values = c("black", "black")) +
  scale_fill_manual(values = c("grey3", "royalblue", "goldenrod")) +
...
### MAKE SURE YOU CHANGE YES TO LATE!!

## Stats

beta2 <- subset(beta, Venom != "Yes" & Voltage == -20)
beta2$Species <- relevel(beta2$Species, ref = "Di")
