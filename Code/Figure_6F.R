...
pdf(file="~/Dropbox/Current_Biology/Gmouse-Nav1.4/Figures/Figure_6/Figure_6F.pdf")
beta <- read.csv(file = "/Users/abhijnaparigi/Dropbox/Current_Biology/Gmouse-Nav1.4/CSV/noc_activation.csv", header = T)
...
current <- subset(beta, Species == "NoC" | Species == "DiDiiiNC" | Species == "Gmouse")
...
peaks$Species <- factor(peaks$Species,levels = c("Gmouse", "NoC", "DiDiiiNC"))
...
ggplot(peaks, aes(x = Species, y = Peak_Diff, fill = Species)) +
  ...
  scale_x_discrete(name = "", labels= c("Gmouse", "NoC", "Triple Mutant")) +
  coord_cartesian(ylim = c(0,0.5)) +
  scale_color_manual(values = c("black", "black")) +
  scale_fill_manual(values = c("grey3", "blue", "goldenrod")) +
...
beta2$Species <- relevel(beta2$Species, ref = "NoC")
beta2$Venom <- relevel(beta2$Venom, ref = "No")
